<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
            <h2 class="text-2xl font-bold text-gray-900 tracking-tight">Edit Buku</h2>
            <p class="text-sm text-gray-500 mt-1">Perbarui informasi buku <span class="font-semibold text-indigo-600">{{ $judul_buku }}</span>.</p>
        </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    ...
                    <div class="relative w-full aspect-[3/4] bg-gray-50 rounded-xl border-2 border-dashed border-gray-300 flex flex-col items-center justify-center overflow-hidden group hover:border-indigo-500 hover:bg-indigo-50/30 transition-all duration-300 cursor-pointer">
                        ...
                        {{-- Logika Tampilan Gambar: Baru -> Lama -> Placeholder --}}
                        @if ($new_cover_image)
                            <img src="{{ $new_cover_image->temporaryUrl() }}" class="w-full h-full object-cover transition duration-500 group-hover:scale-105">
                        ...
                        @elseif ($old_cover_image)
                            <img src="{{ asset('storage/' . $old_cover_image) }}" class="w-full h-full object-cover transition duration-500 group-hover:scale-105">
                            <div class="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition duration-300">
                        ...
                        @else
                            <div class="text-center p-6 group-hover:scale-105 transition duration-300">
                                ...
                                <p class="text-sm font-medium text-gray-700">Upload Cover</p>
                                <p class="text-xs text-gray-400 mt-1">PNG, JPG (Max. 2MB)</p>
                            </div>
                        @endif
                        
                        <input type="file" wire:model="new_cover_image" class="absolute inset-0 opacity-0 cursor-pointer z-10">
                    </div>
                    ...
                    {{-- Progress Bar --}}
                    <div wire:loading wire:target="new_cover_image" class="w-full mt-3">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    ...
                    <div class="space-y-4">
                        <div class="relative group">
                            ...
                            <label for="jumlah_eksemplar_total" class="input-label">Total Stok (Revisi)</label>
                            @error('jumlah_eksemplar_total') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                        </div>
                        <p class="text-xs text-gray-500 -mt-2">Mengubah stok total akan otomatis menyesuaikan stok tersedia.</p>

                        <div class="relative group">
                            <input type="text" id="lokasi_rak" wire:model="lokasi_rak" class="input-field peer" placeholder=" " />
                            <label for="lokasi_rak" class="input-label">Lokasi Rak</label>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        {{-- Penulis (Multi-Select dengan Alpine) --}}
                        <div class="col-span-1 md:col-span-2 relative" 
                             x-data="{
                                 ...
                                 getName(id) { return this.items.find(i => i.id == id)?.name || 'Unknown'; }
                             }">
                            
                            <label class="block text-sm font-medium text-gray-700 mb-2 ml-1">Penulis</label>
                            
                            {{-- Selected Tags --}}
                            <div class="flex flex-wrap gap-2 mb-3 min-h-[2rem]" x-show="selected.length > 0">
                                <template x-for="id in selected" :key="id">
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-indigo-50 text-indigo-700 border border-indigo-100 shadow-sm">
                                        ...
                                        <button type="button" @click="toggle(id)" class="flex-shrink-0 ml-1.5 h-4 w-4 rounded-full inline-flex items-center justify-center text-indigo-400 hover:bg-indigo-200 hover:text-indigo-600 transition">
                            ...
                            {{-- Input Search --}}
                            <div class="relative group">
                                <input type="text" x-model="search" @focus="open = true" @click.away="open = false" class="input-field peer" placeholder=" " />
                                <label class="input-label">Cari & Tambah Penulis...</label>
                                <div class="absolute inset-y-0 right-0 flex items-center px-4 pointer-events-none text-gray-400">
                                ...
                                <div x-show="open" 
                                    ...
                                    <template x-for="item in filteredItems" :key="item.id">
                                        <div @click="toggle(item.id)" class="cursor-pointer px-4 py-2.5 text-sm hover:bg-indigo-50 transition flex items-center justify-between">
                                    ...
                                    <div x-show="filteredItems.length === 0" class="px-4 py-3 text-sm text-gray-500 text-center italic">Penulis tidak ditemukan.</div>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="flex justify-end gap-4 pt-4">
                    <a href="{{ route('buku.index') }}" class="px-6 py-3 bg-white border border-gray-300 text-gray-700 rounded-xl font-semibold hover:bg-gray-50 transition shadow-sm">Batal</a>
