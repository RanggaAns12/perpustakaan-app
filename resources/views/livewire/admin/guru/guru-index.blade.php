<div class="min-h-screen bg-gray-50/50 pb-12" x-data="{ deleteModalOpen: false, deleteId: null }">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8 animate-fade-in-up">
            <div>
                <h1 class="text-3xl font-bold text-gray-900 tracking-tight">Data Guru</h1>
                <p class="text-gray-500 mt-2">Manajemen data pendidik dan tenaga pengajar.</p>
            </div>
            <a href="{{ route('guru.create') }}" class="inline-flex items-center px-5 py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-semibold rounded-xl shadow-lg shadow-indigo-200 hover:shadow-indigo-300 hover:-translate-y-0.5 transition-all duration-200 group">
                <x-heroicon-o-plus class="w-5 h-5 mr-2 group-hover:rotate-90 transition-transform duration-300" /> Tambah Guru
            </a>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-2 mb-8 sticky top-20 z-30 backdrop-blur-xl bg-white/80 transition-all hover:shadow-md">
            <div class="relative group">
                ...
                <input wire:model.live.debounce.300ms="search" type="text" class="block w-full pl-11 pr-4 py-2.5 bg-transparent border-none rounded-xl text-gray-900 placeholder-gray-400 focus:ring-2 focus:ring-indigo-500/20 transition-all" placeholder="Cari nama guru atau NIP..." />
            </div>
        </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-gray-50/50 border-b border-gray-100 text-xs uppercase tracking-wider text-gray-500 font-semibold">
                            <th class="px-6 py-4 rounded-tl-2xl">Nama Guru</th>
                            <th class="px-6 py-4">NIP</th>
                            <th class="px-6 py-4">Kontak & Mapel</th>
                            ...
                            <tr class="group hover:bg-indigo-50/30 transition-colors duration-200">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div class="h-10 w-10 rounded-full bg-gray-100 text-gray-600 flex items-center justify-center font-bold shrink-0 overflow-hidden border border-gray-200">
                                            @if($item->foto_profil)
                                ...
                                <td class="px-6 py-4 text-sm font-mono text-gray-600">{{ $item->nip ?? '-' }}</td>
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-900 font-medium">{{ $item->mata_pelajaran ?? '-' }}</div>
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="flex items-center justify-center gap-2 opacity-80 group-hover:opacity-100 transition-opacity">
                                        <a href="{{ route('guru.edit', $item->guru_id) }}" class="p-2 text-gray-500 hover:text-yellow-600 hover:bg-yellow-50 rounded-lg transition-all"><x-heroicon-o-pencil-square class="w-5 h-5" /></a>
                                        <button @click="deleteModalOpen = true; deleteId = {{ $item->guru_id }}" class="p-2 text-gray-500 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all"><x-heroicon-o-trash class="w-5 h-5" /></button>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr><td colspan="5" class="px-6 py-16 text-center text-gray-500">Data guru belum tersedia.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
            <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/50">{{ $gurus->links() }}</div>
        </div>
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                ...
                                <div class="mt-2"><p class="text-sm text-gray-500">Apakah Anda yakin? Akun login guru ini juga akan dihapus permanen.</p></div>
                            </div>
                        </div>
                    </div>
