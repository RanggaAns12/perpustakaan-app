<div class="min-h-screen bg-gray-50/50 pb-12" x-data="{ deleteModalOpen: false, deleteId: null }">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8 animate-fade-in-up">
            <div>
                <h1 class="text-3xl font-bold text-gray-900 tracking-tight">Data Penerbit</h1>
                <p class="text-gray-500 mt-2">Kelola daftar penerbit dan informasi kontak mereka.</p>
            </div>
            <a href="{{ route('penerbit.create') }}" class="inline-flex items-center px-5 py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-semibold rounded-xl shadow-lg shadow-indigo-200 hover:shadow-indigo-300 hover:-translate-y-0.5 transition-all duration-200 group">
                <x-heroicon-o-plus class="w-5 h-5 mr-2 group-hover:rotate-90 transition-transform duration-300" /> Tambah Penerbit
            </a>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-2 mb-8 sticky top-20 z-30 backdrop-blur-xl bg-white/80 transition-all hover:shadow-md">
            <div class="relative group">
                ...
                <input wire:model.live.debounce.300ms="search" type="text" class="block w-full pl-11 pr-4 py-2.5 bg-transparent border-none rounded-xl text-gray-900 placeholder-gray-400 focus:ring-2 focus:ring-indigo-500/20 transition-all" placeholder="Cari penerbit..." />
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-gray-50/50 border-b border-gray-100 text-xs uppercase tracking-wider text-gray-500 font-semibold">
                            <th class="px-6 py-4 rounded-tl-2xl">Nama Penerbit</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($penerbits as $item)
                            <tr class="group hover:bg-indigo-50/30 transition-colors duration-200">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div class="h-10 w-10 rounded-lg bg-indigo-50 text-indigo-600 flex items-center justify-center shrink-0 border border-indigo-100"><x-heroicon-o-building-office-2 class="w-5 h-5" /></div>
                                        <div>
                                            <div class="font-bold text-gray-900 group-hover:text-indigo-600 transition-colors">{{ $item->nama_penerbit }}</div>
                                            <div class="text-xs text-gray-500 truncate max-w-[200px]">{{ $item->alamat_penerbit ?? 'Alamat belum diisi' }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4"><span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-700 border border-gray-200">{{ $item->kota ?? '-' }}</span></td>
                                <td class="px-6 py-4">
                                    <div class="space-y-1">
                                        @if($item->email_penerbit) <div class="flex items-center gap-2 text-sm text-gray-600"><x-heroicon-o-envelope class="w-4 h-4 text-gray-400" /> {{ $item->email_penerbit }}</div> @endif
                                        @if($item->telepon_penerbit) <div class="flex items-center gap-2 text-sm text-gray-600"><x-heroicon-o-phone class="w-4 h-4 text-gray-400" /> {{ $item->telepon_penerbit }}</div> @endif
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <div class="flex items-center justify-center gap-2 opacity-80 group-hover:opacity-100 transition-opacity">
                                        <a href="{{ route('penerbit.edit', $item->penerbit_id) }}" class="p-2 text-gray-500 hover:text-yellow-600 hover:bg-yellow-50 rounded-lg transition-all"><x-heroicon-o-pencil-square class="w-5 h-5" /></a>
                                        {{-- Trigger Modal --}}
                                        <button @click="deleteModalOpen = true; deleteId = {{ $item->penerbit_id }}" class="p-2 text-gray-500 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all"><x-heroicon-o-trash class="w-5 h-5" /></button>
                                    </div>
                                </td>
                            </tr>
                        @empty
            ...
            <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/50">{{ $penerbits->links() }}</div>
        </div>
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                <h3 class="text-lg leading-6 font-medium text-gray-900">Hapus Penerbit</h3>
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500">Apakah Anda yakin? Buku yang terkait mungkin kehilangan data penerbit.</p>
                                </div>
                            </div>
                        </div>
                    </div>
